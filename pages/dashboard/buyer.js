...
export default function BuyerDashboard() {
  ...
  useEffect(() => {
    if (user) {
      supabase.from('orders').select('*, order_items(*, products(*))').eq('customer_id', user.id).then(({ data }) => setOrders(data || []));
    }
  }, [user]);
  ...
  return (
    <Layout>
      ...
      <p className="mb-8">Привет, {profile?.full_name || 'Покупатель'}!</p>

      {!showApplication ? (
        <>
          <div className="card mb-8">
            <h2 className="text-xl font-semibold mb-4">Стать мастером</h2>
            <p className="text-gray-600 mb-4">Хотите продавать свои работы и мастер‑классы? Отправьте заявку, и мы откроем вам магазин.</p>
            <button onClick={() => setShowApplication(true)} className="btn-primary">Стать мастером</button>
          </div>
              ...
              {orders.map(order => (
                <div key={order.id} className="card">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-sm text-gray-400">{order.status === 'pending' ? 'В обработке' : order.status}</span>
                  </div>
                  <ul className="text-sm">
                    {order.order_items.map(item => (
                      <li key={item.id}>{item.products?.title || 'Товар'} × {item.quantity} — {item.price} ₽</li>
                    ))}
                  </ul>
        ...
        <div className="card max-w-lg">
          <h2 className="text-2xl font-heading font-semibold mb-4">Заявка на становление мастером</h2>
          <form onSubmit={submitApplication} className="space-y-4">
            ...
            <input value={appContact} onChange={e => setAppContact(e.target.value)} placeholder="Контакт для связи (email/телеграм)" className="w-full px-4 py-3 rounded-full border border-gray-200 focus:outline-none focus:border-aurora-green" />
